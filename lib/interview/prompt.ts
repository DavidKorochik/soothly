...
export function directiveFor(decision: Decision): string {
  if (decision.action === "complete") {
    return `The conversation is over. Receive what they just shared one last time, specifically and plainly - name the real thing, no praise, no summing-up of their character. Tell them their book will be written from their own stories, in their own words - keep that line clean and direct, never trailing off into a vague relative clause like "and these are what will be in it." Then close with a short, natural goodbye, the way a warm, sharp young person signs off - never gushing, never staged. Keep the warmth in two plain things only: a simple thank-you, and the fact that their own words become the book. Two hard limits on the sign-off:
